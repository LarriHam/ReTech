...
<div class="px-4 py-4">
    ...
    <div class="mb-6">
        ...
        <input type="user_id" id="user_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{$comment->user->name}}" disabled>
    </div>

    <button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"><a href="{{route('ordinary_user.comments.edit', $comment->id)}}">Edit</a></button>
    
    <form method="POST" action="{{ route('ordinary_user.comments.destroy', $comment->id) }}">
            @csrf
            @method('DELETE')
